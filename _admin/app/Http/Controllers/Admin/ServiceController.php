...
//		$this->middleware('auth');
	...
	public function index(){
		...
		$list['services'] = Services::where('shop_id',$list['adminuser']['shop_id'])->get();
	...
	public function service($id = null){
		$shop = new Shops();
		$list['adminuser'] = Admins::find(Auth::id());
		$list['service'] = Services::where('id', $id)->where('shop_id',$list['adminuser']['shop_id'])->first();
		$list['staffs'] = Staffs::where('shop_id',$list['adminuser']['shop_id'])->get();
	...
	public function update($id = null){
		...
		$list['service']->shop_id = $list['adminuser']['shop_id'];
	...
	public function reservation_conform(){
		...
		$list['tickets'] = Tickets::where('client_id', Auth::user()->id)->where('shop_id',$list['adminuser']['shop_id'])->get();
	...
	public function reservation_comp(){
		...
		$Tickets = Tickets::where('client_id', Auth::user()->id)->where('shop_id',$list['adminuser']['shop_id'])->get();
		$Tickets = $Tickets[0];
	...
	public function ticketadd(){
       ...
       if(!$db){
       	$db = new Tickets();
       	$db->shop_id = $list['adminuser']['shop_id'];
	...
	public function ticketuse(){
       ...
       $db = Tickets::where('client_id', Auth::user()->id)->where('shop_id', $list['adminuser']['shop_id'])->update(['count' => $count-1]);
       if(!$db){
       	return false;
       }
	}
