@extends('user.layout')
    @section('content')
        ...
        <div class="mb-3">
            <form action="{{ url( 'user/receive_confort' ) }}" method="GET">

            <div class="mb-3">通知メールの受信</div>
            <div  class="mb-3">
                {{ $user->is_receive }}
                <input name="receive" type="radio" <?php if($user->is_receive == 1){?>selected="selected"  <?php }?> value="1">有効
                <input name="receive" type="radio" <?php if($user->is_receive == 0){?>selected="selected" <?php }?> value="0">無効
            </div>
